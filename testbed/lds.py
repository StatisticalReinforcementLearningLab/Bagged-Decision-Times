...
class SLDS(object):
    ...
    def initialize(self, datas, inputs=None, masks=None, tags=None,
        ...
        inputs_none = [np.zeros((data.shape[0], 0)) for data in datas]
        self.emissions.initialize(datas, inputs_none, masks, tags)
        self.emissions.inv_etas[:,...] = np.log(np.var(datas))

        # Get the initialized variational mean for the data
        xs = [self.emissions.invert(data, input, mask, tag)
              for data, input, mask, tag in zip(datas, inputs_none, masks, tags)]
    ...
    def _laplace_neg_expected_log_joint(self,
        ...
        log_likes = self.dynamics.log_likelihoods(x, input, x_mask, tag)
        input_none = np.zeros((data.shape[0], 0))
        log_likes += self.emissions.log_likelihoods(data, input_none, mask, tag, x)
    ...
    def _laplace_neg_hessian_params(self, data, input, mask, tag, x, Ez, Ezzp1):
        ...
        J_obs = self.emissions.\
            neg_hessian_log_emissions_prob(data, input_none, mask, tag, x, Ez)
...
class LDS(SLDS):
    ...
    def __init__(self, N, D, *, M=0,
        ...
        if isinstance(emissions, str):
            ...
            emission_kwargs = emission_kwargs or {}
            emissions = emission_classes[emissions](N, 1, D, M=0,
