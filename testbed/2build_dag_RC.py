...
digits = 3
C_shift = np.round(np.mean(dat_org['C']), digits)
C_scale = np.round(np.std(dat_org['C']), digits)
M_shift = np.round(np.mean(dat_org['M']), digits)
M_scale = np.round(np.std(dat_org['M']), digits)
E_shift = np.round(np.mean(dat_org['E']), digits)
E_scale = np.round(np.std(dat_org['E']), digits)
O_shift = np.round(np.mean(dat_org['O']), digits)
O_scale = np.round(np.std(dat_org['O']), digits)
...
C_limit = [np.round(np.min(dat_org['C']), digits), np.round(np.max(dat_org['C']), digits)]
M_limit = [np.round(np.min(dat_org['M']), digits), np.round(np.max(dat_org['M']), digits)]
E_limit = [np.round(np.min(dat_org['E']), digits), np.round(np.max(dat_org['E']), digits)]
R_limit = [np.round(np.min(dat_org['R']), digits), np.round(np.max(dat_org['R']), digits)]
O_limit = [np.round(np.min(dat_org['O']), digits), np.round(np.max(dat_org['O']), digits)]
...
with open(file_params_std, 'w') as file:
    file.write(json.dumps(std_params))
...
for i, userid in enumerate(userid_all):
    ...
    model_C = RidgeCV(alphas=alpha_l2_list, fit_intercept=False, cv=ncv)
    model_C.fit(C_cond, C_h)
    alpha_C_l2 = model_C.alpha_
    theta_C_mean = model_C.coef_
    pred_C = model_C.predict(C_cond)
    resid_C = C_h - pred_C
    sigma2_C_mean = np.var(resid_C)
    ...
    env_para = {
        ...
        'theta_C': np.round(theta_C_mean, digits).tolist(),
        ...
        'resid_C': np.round(resid_C, digits).tolist(),
