...
class BRLSVI():
    def __init__(self, env_config: EnvConfigBase, lam_beta: float, 
        ...
        self.dAC = [1 + self.dE + self.dR + self.dC + k * self.dC + k * self.dA for k in range(self.K)]
        self.dX = (
            2 + (self.K - 1) * self.dC + (self.K - 1) * self.dA 
    ...
    def get_Q(
        ...
        comb_X_next_main = np.hstack([
            ...
            comb_C_prev_next, comb_A_prev_next, 
        ...
        for k in range(self.K - 1, -1, -1):
            ...
            comb_inter_h[:, sum(self.dAC[:k]):sum(self.dAC[:k + 1])] = np.hstack([
                ...
                comb_A_h * comb_C_prev_h[:, :(k * self.dC)],
            ...
            comb_inter_h_A0[:, sum(self.dAC[:k]):sum(self.dAC[:k + 1])] = np.hstack([
                ...
                comb_A0 * comb_C_prev_h[:, :(k * self.dC)],
            ...
            comb_inter_h_A1[:, sum(self.dAC[:k]):sum(self.dAC[:k + 1])] = np.hstack([
                ...
                comb_A1 * comb_C_prev_h[:, :(k * self.dC)],
