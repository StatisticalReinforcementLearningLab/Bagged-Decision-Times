...
class Env:
    ...
    def gen_Rd_mean(self, Md, Ed, Rdm1, Ad, n = 1):
        size = (1, ) if n == 1 else (n, 1)
        ...
        ## find the mean value conditional on the predictors
        R_cond = np.hstack([
            np.ones(size), Md, Ed, Rdm1, Ad
        ])
        R_cond = R_cond.reshape(n, 1 + self.K * self.dM + self.dE + self.dR + self.K * self.dA)
        Rd = np.matmul(R_cond, self.theta_R).reshape(-1) 
        return Rd


    def gen_Rd(self, Md, Ed, Rdm1, Ad, d):
        R_mean = self.gen_Rd_mean(Md, Ed, Rdm1, Ad)
        if self.noise == 'sequential':
            ## cyclically sample the noise term from the residual vector
