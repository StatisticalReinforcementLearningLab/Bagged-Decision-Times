...
class Env:
    def __init__(self, env_config: EnvConfigBase, noise='sequential'):
        ...
        self.theta_M = [t.reshape(-1, self.dM) for t in env_config.theta_M]
    ...
    def gen_Mh_mean(self, Edm1, Rdm1, Ch, Ah, Md, k, n = 1):
        size = (1, ) if n == 1 else (n, 1)
        dCM = 1 + self.dE + self.dR + self.dC + self.dM * k
        ## find the mean value conditional on the predictors
        M_cond = np.hstack([
            np.ones(size), Edm1, Rdm1, Ch, Md[:k].reshape(-1),
        ]).reshape(n, dCM)
        Mh_main = np.matmul(M_cond[:, :4], self.theta_M[k][:4, :]).reshape(-1) 
        Mh_trt = (Ah * np.matmul(M_cond, self.theta_M[k][4:, :])).reshape(-1)
        # Mh_trt = np.maximum(Mh_trt, 0)
        return Mh_main + Mh_trt


    def gen_Mh(self, Edm1, Rdm1, Ch, Ah, Md, d, k):  ## currently only support n=1
        M_mean = self.gen_Mh_mean(Edm1, Rdm1, Ch, Ah, Md, k)
        if self.noise == 'sequential':
            ## cyclically sample the noise term from the residual vector
            M_noise = self.resid_M[k][d % self.resid_M.shape[1]]
            ## if this residual is missing, randomly sample from observed residuals at time k
            if np.isnan(M_noise):
                M_noise = rd.choice(self.resid_M[k][~np.isnan(self.resid_M[k])], size=1)
        elif self.noise == 'random':
            M_noise = rd.choice(self.resid_obs_M, size=1) ## ignore d and k
