...
class Env:
    ...
    def gen_Ch_mean(self, Edm1, Rdm1, n = 1):
        size = (1, ) if n == 1 else (n, 1)
        dCC = 1 + self.dE + self.dR
        ## find the mean value conditional on the predictors
        C_cond = np.hstack([
            np.ones(size), Edm1, Rdm1
        ]).reshape(n, dCC)
        Ch = np.matmul(C_cond, self.theta_C).reshape(-1) 
        return Ch


    def gen_Ch(self, Edm1, Rdm1, d, k):  ## currently only support n=1
        C_mean = self.gen_Ch_mean(Edm1, Rdm1)
